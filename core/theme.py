...
def detect_terminal_background() -> str:
    ...
    """
    # Method 1: Check COLORFGBG environment variable (used by many terminals)
    ...
    # Method 2: Check TERM_PROGRAM for known terminals
    term_program = os.environ.get('TERM_PROGRAM', '').lower()
    if 'vscode' in term_program:
        # VS Code integrated terminal - assume dark by default
        return 'dark'
    elif 'apple_terminal' in term_program:
        # macOS Terminal - check more specific indicators
        pass
    
    # Method 3: Check if running in TTY (usually dark)
    if os.environ.get('TERM', '') == 'linux':
        return 'dark'
    
    # Method 4: Check for light/dark theme indicators in environment
